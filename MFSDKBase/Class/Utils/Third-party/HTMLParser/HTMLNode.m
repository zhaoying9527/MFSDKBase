...
NSString * getAttributeNamed(xmlNode * node, const char * nameStr)
{
	for(xmlAttrPtr attr = node->properties; NULL != attr; attr = attr->next)
	{
		if (NULL != attr->name && strcmp((char*)attr->name, nameStr) == 0)
		{				
			for(xmlNode * child = attr->children; NULL != child; child = child->next)
			{
...
-(NSArray*)getAttributes
{
    ...
    NSString *fullAttribute = @"";
    NSMutableArray *array = [NSMutableArray array];
    for(xmlAttrPtr attr = _node->properties; NULL != attr; attr = attr->next)
    {
        for(xmlNode * child = attr->children; NULL != child; child = child->next)
        {
            attributeId = [NSString stringWithCString:(char*)attr->name encoding:NSUTF8StringEncoding];
            attributeValue = [NSString stringWithCString:(void*)child->content encoding:NSUTF8StringEncoding];
            fullAttribute = [NSString stringWithFormat:@"%@=%@", attributeId,attributeValue];
            [array addObject:fullAttribute];
        }
    }
    return array;
}
...
HTMLNodeType nodeType(xmlNode * _node)
{
    ...
    else if (strcmp(tagName, "button") == 0)
        return HTMLButtonNode;
    else if (strcmp(tagName, "label") == 0)
        return HTMLLabelNode;
