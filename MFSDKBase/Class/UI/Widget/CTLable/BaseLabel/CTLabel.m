...
static dispatch_queue_t ct_attributed_label_parse_queue;
static dispatch_queue_t get_ct_attributed_label_parse_queue() \
{
    if (ct_attributed_label_parse_queue == NULL) {
        ct_attributed_label_parse_queue = dispatch_queue_create("com.ct.parse_queue", 0);
    }
    return ct_attributed_label_parse_queue;
}
...
@property (nonatomic,strong)    CTLabelURL *touchedLink;
...
- (void)commonInit
{
    ...
    _textFrame              = nil;
    _linkColor              = [UIColor colorWithRed:0/255.0 green:170/255.0 blue:238/255.0 alpha:1];
    ...
    _detectLinkType         = CTLabelLinkDetectNone;
...
- (void)setFont:(UIFont *)font
{
    if (font && _font != font)
    {
        ...
        [self resetFont];
        for (CTAttributedLabelAttachment *attachment in _attachments)
        {
...
- (NSAttributedString *)attributedStringForDraw
{
    if (_attributedString)
    {
        ...
        for (CTLabelURL *url in _linkLocations)
        {
            ...
            [drawString setUnderlineStyle:url.underLineForLink ? kCTUnderlineStyleSingle : kCTUnderlineStyleNone
                                 modifier:kCTUnderlinePatternSolid
                                    range:url.range];
            if(url.underLineColor){
                [drawString setUnderlineColor:url.underLineColor range:url.range];
            }
        }
        return drawString;
    }
...
- (CTLabelURL *)urlForPoint: (CGPoint)point
{
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        if (CGRectContainsPoint(rect, point))
        {
            ...
            CTLabelURL *url = [self linkAtIndex:idx];
            if (url)
            {
                return url;
            }
...
{
    CTLabelURL *url = [self urlForPoint:point];
...
- (CTLabelURL *)linkAtIndex:(CFIndex)index
{
    for (CTLabelURL *url in _linkLocations)
    {
...
- (void)appendAttachment: (CTAttributedLabelAttachment *)attachment
{
    attachment.fontAscent                   = _fontAscent;
    attachment.fontDescent                  = _fontDescent;
             ...
             margin: (UIEdgeInsets)margin
{
    [self appendImage:image
              maxSize:maxSize
               margin:margin
            alignment:CTImageAlignmentBottom];
}
...
{
    CTAttributedLabelAttachment *attachment = [CTAttributedLabelAttachment attachmentWith:image
                                                                                   margin:margin
                                                                                alignment:alignment
                                                                                  maxSize:maxSize];
...
- (void)appendView: (UIView *)view
         ...
         alignment: (CTImageAlignment)alignment
{
    CTAttributedLabelAttachment *attachment = [CTAttributedLabelAttachment attachmentWith:view
                                                                                     margin:margin
                                                                                  alignment:alignment
                                                                                    maxSize:CGSizeZero];
    [self appendAttachment:attachment];
...
- (void)addCustomLink: (id)linkData
            ...
            underLine: (BOOL)line
{
    CTLabelURL *url = [CTLabelURL urlWithLinkData:linkData
                                            range:range
                                            color:color
                                 underLineForLink:line];
...
{
    [CTLabelURL setCustomDetectMethod:block];
}
...
{
    UIColor *highlightColor = self.touchedLink.highlightColor?:self.highlightColor;
    
    if (self.touchedLink && highlightColor)
    {
                    ...
                    _lineBreakMode == kCTLineBreakByTruncatingTail)
                {
                    ...
                    if (lastLineRange.location + lastLineRange.length < attributedString.length)
                    {
                        ...
                        NSDictionary *tokenAttributes = [attributedString attributesAtIndex:truncationAttributePosition
                                                                             effectiveRange:NULL];
                        NSAttributedString *tokenString = [[NSAttributedString alloc]
...
- (void)drawAttachments
{
    ...
    for (CFIndex i = 0; i < numberOfLines; i++)
    {
        ...
        for (CFIndex k = 0; k < runCount; k++)
        {
            ...
            CTAttributedLabelAttachment* attributedImage = (CTAttributedLabelAttachment *)CTRunDelegateGetRefCon(delegate);
...
- (BOOL)onLabelClick:(CGPoint)point
{
    ...
    if (linkData)
    {
        if (_delegate && [_delegate respondsToSelector:@selector(ctAttributedLabel:clickedOnLink:)])
        {
            [_delegate ctAttributedLabel:self clickedOnLink:linkData];
        }
        else
        {
            NSURL *url = nil;
...
- (void)recomputeLinksIfNeeded
{
    ...
    BOOL sync = length <= CTMinAsyncDetectLinkLength;
               ...
               sync:(BOOL)sync
{
    ...
    {
        _ignoreRedraw = YES;
        NSArray *links = [CTLabelURL detectLinks:text linkType:_detectLinkType];
        block(links);
        _ignoreRedraw = NO;
    }
    else
    {
        dispatch_async(get_ct_attributed_label_parse_queue(), ^{
            NSArray *links = [CTLabelURL detectLinks:text linkType:_detectLinkType];
...
- (void)addAutoDetectedLink: (CTLabelURL *)link
{
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    CTLabelURL *touchedLink = [self urlForPoint:point];
